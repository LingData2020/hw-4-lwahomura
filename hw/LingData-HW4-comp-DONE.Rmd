...
```{r}
count(distinct(mono_socio[,1])) # 1 question

count(filter(mono_socio, QuestionCategory == "sex", Responce == "male")) # 2 question
count(filter(mono_socio, QuestionCategory == "sex", Responce == "female"))

distinct(filter(mono_socio, QuestionCategory == "education")[,4]) #3 question

filter(mono_socio, QuestionCategory == "education") %>% group_by(Responce) %>% summarise(count = n()) # 4 question

count(filter(mono_socio, QuestionCategory == "handedness", Responce == "right")) # 5 question
count(filter(mono_socio, QuestionCategory == "handedness", Responce == "left"))
```


Compare you overview with that reported in Table 1 of the article. 
```
The results are neraly the same as in Table 1; the difference is in participants' education level
```

Sometimes replication data provided by authors does not allow on to reproduce their results. Let's look at another dataframe, `mono`, that contains results of experiment 1. 
...
```
I'll use a density plot because RT is a numeric value; although it's discrete it's values are close to each other so i think we can assume them to be non-discrete 
```
...
```
It could look like a normal distribution, but it has a very long right tail because of a several outliers in data
```
...
```{r}
mutated <- mono %>% mutate(RTlog = log10(RT))
```
...
```
It looks much more like a normal distribution - the outliers now have much lowe impact on the whole plot; though it's a bit twisted to the left side
```
...
# filter here
mono1 <-
  mutated %>% filter(RT >= 600, abs(RTlog - mean(RTlog)) <= 3 * sd(RTlog), TypeOfStimuli != "FillerAcceptable", TypeOfStimuli != "FillerUnacceptable") %>%
...
```{r}
summarized1 <- mono1 %>% group_by(TypeOfStimuli, AcceptabilityJ, WordOrder) %>% summarise(Count = n())
ggplot(summarized1, aes(x = TypeOfStimuli, fill = AcceptabilityJ, y = Count)) + 
  geom_col(colour = "black", position = "dodge") + facet_grid(cols = vars(WordOrder))
```
...
```{r}
summarized2 <- mono1 %>% group_by(AcceptabilityJ, WordOrder) %>% summarise(Count = n())
ggplot(summarized2, aes(x = WordOrder, fill = AcceptabilityJ, y = Count)) + 
...
ggplot(mono1, aes(x = AcceptabilityJ, fill = WordOrder, y = RTlog)) + 
  geom_violin(position = dodge) + geom_boxplot(width=0.3, position = dodge)
```
...
```{r}
mean_rtlog_long <- mono1 %>% group_by(ParticipantID, WordOrder) %>% summarise(RTlog = mean(RTlog))
mean_rtlog_long
```
...
```
No, it shouldn't be rejected because p-value is more than 0.05
```

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test?

```
We can say that logarithms of reaction time for different stimuli are nearly the same which means that we can say they belong to a same sample
```
...
```{r}
mean_rtlog <- mean_rtlog %>% mutate(diff = Congruent - Incongruent)
zeros <- double(140)
t.test(mean_rtlog$diff, zeros, alternative = "two.sided")
```
...
```
Now it seems like the means are different so they belong to different samples 
```

How can you interpret difference with the result of 2.3?

```
The difference comes because if we use independent t-test we assume that all the measures were independent and that we first performed test on one group of people for Congruent stimuli, then the other for Inongruent stimuli; though we had only one group so every person is associated with two results 
```
...
```
We used one-way ANOVA because we have only one factor and more than two groups.  
We decline the hypothesis that there is no significant difference for different conditions - p-value is less than 0.05. So we say that there is a difference in the results depending on the type of stimula
```
...
```
We can see that only the pair Size-Nationality and Shape-Color is close to being the one where is no significant difference between the results; so there is no matter is we use Size-Nationality stimula or Shape-Color; the other two pair have significant difference  between logarithms - so it matters if we use SubjectiveComment-Material stimula or any other one
```
